...
def contact_list(request):
	contacts = Contact.objects.filter(user=request.user,~slug=str(request.user)+'RPiDefaultUser')
	return render(request, 'mobile/contacts.html', {'contacts':contacts})

@login_required
def custom_contact_list(request,slug):
	contacts = Contact.objects.filter(user=request.user,original_name=slug,~slug=str(request.user)+'RPiDefaultUser')
	return render(request, 'mobile/contacts.html', {'contacts':contacts})
